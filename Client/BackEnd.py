...
class Foto:
    ...
    def recortar_be(self, bytes, click1, click2, Qsize):
        matriz = self.get_rgb(bytes)
        alto_label1 = Qsize.height()
        ancho_label1 = Qsize.width()
        alto_imagen = len(matriz)
        ancho_imagen = len(matriz[0])

        ratio = alto_imagen / alto_label1

        desde_x = min([click1.x(), click2.x()])
        hasta_x = max([click1.x(), click2.x()])
        desde_y = min([click1.y(), click2.y()])
        hasta_y = max([click1.y(), click2.y()])
